...
<p align="center">
  <img src="docs/demo.png" alt="Демо бота: фото кота превращается в космонавта-кота на Луне" width="300">
</p>
...
- **Сила референса** (`input_fidelity`) поддерживается не всеми моделями: `gpt-image-1` — да,
  `gpt-image-2` — нет. Бот отправляет параметр и, если модель его отвергает, прозрачно повторяет
  запрос без него — настройка никогда не ломает генерацию, просто не влияет на модели, которые её
  игнорируют.
